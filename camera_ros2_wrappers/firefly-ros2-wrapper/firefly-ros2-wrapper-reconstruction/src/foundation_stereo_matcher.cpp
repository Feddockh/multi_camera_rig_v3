    ...
    : config_(config)
{
    ...
    runner_ = std::make_unique<TrtRunner>(config_.engine_path);
...
{
    auto mode = toLower(config_.disp_filter_mode);
...
void FoundationStereoMatcher::process(const cv::Mat &left_bgr, const cv::Mat &right_bgr, cv::Mat &disparity)
{
    ...
    // Run inference
    runner_->run(left_nchw_.data(), right_nchw_.data(), disp_buffer_.data(), disp_buffer_.size());

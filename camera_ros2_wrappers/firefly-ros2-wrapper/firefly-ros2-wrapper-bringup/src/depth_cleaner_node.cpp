 ...
 * - Valid depth values (0 < depth <= max_depth) are preserved
 * - Invalid values (NaN, Inf, 0, > max_depth) are replaced with invalid_depth_value
 * - Optionally downsamples invalid pixels to reduce redundant ray casting
 * 
 ...
 * while minimizing computational overhead from redundant rays.
 */
...
private:
    void depthCallback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
        ...
        // Mask for valid depth: 0 < depth <= max_depth
        cv::Mat valid_mask = (depth > 0.0f) & (depth <= static_cast<float>(max_depth_));
        ...
        // 2. Set non-sampled invalid pixels to 0
        cv::Mat non_sampled_invalid = invalid_mask & ~stride_mask;
        depth.setTo(0.0f, non_sampled_invalid);
        ...
        if (!preserve_valid_pixels_ && invalid_pixel_stride_ > 1)
        {
            ...
            depth.setTo(0.0f, non_sampled_valid);
        }
